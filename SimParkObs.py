...
def CarSim(options,ModelPredictiveControl):
    ...
    predicted_info=[state_i]  #this 3D array will have the values of states in all the 250 iterations
    ...
    for i in range(1,sim_total+1):
        ...
        y = mpc.plant_model(state_i[-1],mpc.dt,u[0],u[1])  #y= output of the system(x,y,velocity,angle) NOTE:Only one input value is executed out of the 20 predicted ones
        predicted_states=np.array([y])
        for k in range(1,mpc.horizon):
            predictions=mpc.plant_model(predicted_states[-1],mpc.dt,u[2*k],u[2*k+1])          #predcitons will have row matrix of preiction of the very next state
            predicted_states = np.append(predicted_states,np.array([predictions]),0)    #this row matrix will be added as a row in predicted_state matrix which will have 20 rows
                                                                                        #when it gets out of the loop
        predicted_info+=[predicted_states]                      #predicted_info will have the 250 matrices of predicted states when oit gets out of the loop
        state_i = np.append(state_i,np.array([y]),0)            #the state when we getny executing the first predicted input is now stored as a row in matrix state_i
        u_i = np.append(u_i, np.array([(u[0],u[1])]),0)         #the one exectuted input value is stored in u_i 
...
#########Display#################
    ...
    car_width= 1.0
    patch_car = mpatches.Rectangle((0,0), car_width, 2.5, fc = 'k', fill=False)  #creating a rectangle to repesent a car
    patch_goal = mpatches.Rectangle((0,0),car_width, 2.5, fc='b',ls='dashdot', fill=False) #reacting a rectangle to represemt the parking space


    ax.add_patch(patch_car)   #add car to the plot
    ax.add_patch(patch_goal)  #add parking space to the plot

    predict, =ax.plot([],[],'r--',linewidth=1)  #visual representation of pedicted path


    

    if obstacle:
        patch_obs=mpatches.Circle((mpc.x_obs, mpc.y_obs),0.5)  #add a circular obstacle in the pat
    ...
    def car_patch_pos(x, y, psi):               #The origin of the car is on the top left hand
        ...
        x_new = x - np.sin(psi)*car_width/2     # shift the car a little upward to aline it 
    ...
    def update_plot(num):
        ...
        predict.set_data(predicted_info[num][:,0],predicted_info[num][:,1])  #visual representation of predicted path
    ...
    car_animation= animation.FuncAnimation(fig, update_plot,frames=range(1,len(state_i)),interval=100,repeat=True, blit=False)
